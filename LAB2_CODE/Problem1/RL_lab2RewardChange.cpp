...
void policy_evaluation(){
    ...
    printf("\n");
    double up[2],right[2],down[2],left[2];
    ...
    while(1)
    {
        ...
        for(int i=0;i<MAX_N;i++)
        {
            for(int j=0;j<MAX_N;j++){
                ...
                up[1]=(discount)*((i-1<0)?vk[i][j]:((i-1==1 && j==1)?vk[i][j]:vk[i-1][j]));
                right[1]=(discount)*((j+1>=MAX_N)?vk[i][j]:((i==1 && j+1==1)?vk[i][j]:vk[i][j+1]));
                down[1]=(discount)*((i+1>=MAX_N)?vk[i][j]:((i+1==1 && j==1)?vk[i][j]:vk[i+1][j]));
                left[1]=(discount)*((j-1<0)?vk[i][j]:((i==1 && j-1==1)?vk[i][j]:vk[i][j-1]));
                for(int k=0;k<policy[i][j].size();k++){
